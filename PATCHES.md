...
| 29 | turbo-persistence | make the crate correct and testable on strict wasi hosts. `commit_current` fsync'd the database *directory* (`File::open(dir).sync_data()`), already skipped on Windows; `fd_sync` on a directory fd is implementation-defined in wasi preview1 and rejected by strict hosts (wasmtime → EBADF), so skip it on wasm too (wasi durability is the host FS's job). Also target-gate the `criterion` dev-dep to `not(target_family="wasm")` so the test binary builds for wasm (codspeed-criterion-compat hard-errors on wasi via rayon; benches never run under `cargo test`). Surfaced by `scripts/wasi-test.sh`, which runs the suite on `wasm32-wasip1-threads`; 60/61 turbo-persistence tests then pass (the one failure is an alloc-failure in a max-size compaction stress test — the wasm32 4GB ceiling) |
